# Agent Operating Manual — Advanced (8-GPU multi-campaign mode)

> **For single-kernel optimization, use [`program.md`](program.md) instead.** This file is the operating manual for the original Genesis-on-MI300X integration: 8 agents in parallel, each working a "campaign" (group of kernels) defined in `kernels/<campaign>/target.json`, sharing a cross-agent loss log + a watchdog daemon. See [USAGE.md](USAGE.md) for setup. To trigger this mode, the human's first message will reference this file by name (`read program-advanced.md`).
